...
def inspect(
    ...
    kv(
        f"Task {task.id}",
        {
            ...
            "chamadas de modelo": (result.model_calls if result else 0),
            "tokens (in/out)": (
                f"{result.tokens.get('input', 0)}/{result.tokens.get('output', 0)}" if result else "-"
            ),
            "custo": (f"{result.cost:.6f}" if result else "-"),
            "erro": task.error or "-",
        },
    )
...
def _render(runtime, task: Task, as_json: bool = False) -> None:
    ...
    lines = [
        ...
        f"custo    : {(f'{result.cost:.6f}' + f' ({result.model_calls} chamadas)') if result else '-'}",
    ]
    panel(
        "Task executada",
        "\n".join(lines),
