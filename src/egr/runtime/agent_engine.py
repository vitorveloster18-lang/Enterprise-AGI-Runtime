...
class AgentEngine:
    ...
    def plan(self, task: Task, agent: AgentSpec) -> Plan:
        ...
        try:
            ...
            # Orçamento estourado não é falha de modelo: é decisão de governança.
            raise
    ...
    def _fail(self, task: Task, agent: AgentSpec, reason: str, error_type: EventType) -> Task:
        """Encerra a task como falha governada (orçamento, política, etc.)."""
        ...
        self.runtime.audit.record(
            ...
            agent_id=agent.id,
            environment=str(task.environment),
            payload={"task": task.id, "reason": reason},
        )
        self.runtime.audit.record(
    ...
    def _finish(self, task: Task, agent: AgentSpec, plan: Plan, started: float) -> Task:
        ...
        usage = self.runtime.usage.totals(task_id=task.id)
        task.result.cost = usage["total_cost"]
        task.result.tokens = {
            "input": usage["input_tokens"],
            "output": usage["output_tokens"],
        }
        task.result.model_calls = usage["calls"]
